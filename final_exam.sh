...
function main() {
    test -z "$api_question_result" && {
        echo "请提供期末考试的答案地址并填写到本脚本的api_question_result变量中"
        echo "格式:https://fskjxy.web2.superchutou.com/xxx/xxx/getxxx"
        exit 1
    }
    # 获取学生ID
    curl_student_id=$(curl 'https://fskjxy.web2.superchutou.com/service/eduSuper/StudentinfoDetail/GetStudentDetailRegisterSet' -H "$header_accept" -H "$header_accept_language" -H "$header_access_control_allow_origin" -H "$header_cache_control" -H "$header_connection" -H "$header_content_type" -H "$header_cookie" -H "$header_user_agent" --compressed --insecure -s)

    test "$(echo "$curl_student_id" | jq -r '.Message')" == "请先登录" && {
        echo "cookie无效"
        exit 1
    }
    ...
    # 获取课程列表
    curl_std_curriculum_list=$(curl "https://fskjxy.web2.superchutou.com/service/eduSuper/Specialty/GetStuSpecialtyCurriculumList?StuDetail_ID=${StuDetail_ID}&IsStudyYear=1&StuID=${StuID}" -H "$header_accept" -H "$header_accept_language" -H "$header_access_control_allow_origin" -H "$header_cache_control" -H "$header_connection" -H "$header_content_type" -H "$header_cookie" -H "$header_user_agent" --compressed --insecure -s)
    ...
    # 确定考试序号
    echo
    echo 在选择之前,请先进入期末考试页面,各计时开始后,再选择序号,并回车
    ...
    # 获取 期末考试的信息
    final_exam_info=$(curl "https://fskjxy.web2.superchutou.com/service/eduSuper/Question/GetFinalExamPaperView?StuDetail_ID=${StuDetail_ID}&Curriculum_ID=${curriculum_id}&Examination_ID=0" -H "$header_accept" -H "$header_accept_language" -H "$header_access_control_allow_origin" -H "$header_cache_control" -H "$header_connection" -H "$header_content_type" -H "$header_cookie" -H "$header_user_agent" --compressed --insecure -s)
    ...
    exam_question=$(echo "$curl_question" | jq '.Data.QuestionType[].Question')
    ...
    for ((i = 0; i < real_question_length; i++)); do
        ...
        # echo "题目ID:${real_id} 题目:${real_title}"
    ...
    # 自动提交/交卷
    # while true; do
    #     curl_SubmitExamPractice=$(curl 'https://fskjxy.web2.superchutou.com/service/eduSuper/Question/SubmitExamPractice' --data-raw "$real_answare_compain" -H "$header_accept" -H "$header_accept_language" -H "$header_access_control_allow_origin" -H "$header_cache_control" -H "$header_connection" -H "$header_content_type" -H "${header_cookie}; themeName=default" -H "$header_user_agent" --compressed --insecure -s)
