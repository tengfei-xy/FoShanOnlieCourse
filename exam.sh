...
function main() {
    ...
    for ((z = 0; z < exam_paper_list_length; z++)); do
        ...
        # 获取 阶段测评答案
        curl_exam_result=$(curl "https://fskjxy.web2.superchutou.com/service/eduSuper/Question/GetExamPaperResult?busId=${exam_paper_id}&resultId=${result_id}" -H "$header_accept" -H "$header_accept_language" -H "$header_access_control_allow_origin" -H "$header_cache_control" -H "$header_connection" -H "$header_content_type" -H "$header_cookie" -H "$header_user_agent" --compressed --insecure -s)
        exam_question=$(echo "$curl_exam_result" | jq '.Data.QuestionType[].Question')

        # 获取 阶段测评的问题
        curl_question=$(curl "https://fskjxy.web2.superchutou.com/service/eduSuper/Question/GetExamPaperQuestions?examPaperId=${exam_paper_id}&type=2&StuDetail_ID=${StuDetail_ID}&StuID=${StuID}&Examination_ID=0&Curriculum_ID=${curriculum_id}" -H "$header_accept" -H "$header_accept_language" -H "$header_access_control_allow_origin" -H "$header_cache_control" -H "$header_connection" -H "$header_content_type" -H "$header_cookie" -H "$header_user_agent" --compressed --insecure -s)
        # 已经在下列命令中删除了问题标题
